...
export function BangumiCard({ bangumi, className, style, animate = true, onClick }: BangumiCardProps) {
  const { bangumi: data, subscription } = bangumi;
    ...
    >
      ...
      >
        ...
        <div className="relative aspect-2/3 overflow-hidden">
          <img
            src={data.posterUrl || "/placeholder.png"}
            alt={data.titleChinese}
          ...
          <div className="absolute inset-x-0 bottom-0 p-3 space-y-2">
            ...
            <div className="flex items-center gap-2 text-xs">
              {data.season > 0 && (
                <span className="glass-badge">第{data.season}季</span>
              )}
              <span className="glass-badge">更新至{subscription.currentEpisode}集</span>
            </div>
          </div>
        </div>
      </div>
