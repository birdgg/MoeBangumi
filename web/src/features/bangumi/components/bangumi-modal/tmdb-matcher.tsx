...
interface TmdbDisplayItem extends TmdbSearchItem {
...
interface TmdbMatcherProps {
  onChange: (show: TmdbDisplayItem | null) => void;
...
export function TmdbMatcher({
  ...
  const [selected, setSelected] = React.useState<TmdbDisplayItem | null>(null);
  ...
  // Transform TmdbSearchItem to TmdbDisplayItem
  const results = React.useMemo(() => {
    if (!data || !Array.isArray(data)) return [];
    return data.map((item: TmdbSearchItem): TmdbDisplayItem => ({
      ...item,
      external_id: item.id.toString(),
      title_chinese: keyword,
      title_original: keyword,
    }));
  }, [data, keyword]);
  ...
  const displayValue =
    selected ??
    (initialTmdbId
      ? ({ id: parseInt(initialTmdbId, 10), external_id: initialTmdbId, title_chinese: keyword || "TMDB" } as TmdbDisplayItem)
      : null);
  ...
  const handleSelect = (show: TmdbDisplayItem) => {
    setSelected(show);
    onChange(show);
    setOpen(false);
  };
          ...
          <div className="relative z-50">
            <div className="absolute top-0 left-0 right-0 max-h-64 overflow-y-auto rounded-lg border bg-popover p-1 shadow-lg animate-in fade-in-0 zoom-in-95 slide-in-from-top-2 duration-200">
              {results.length === 0 ? (
                ...
                results.map((show: TmdbDisplayItem) => (
                  <button
