//! Clearable value object for optional field updates.
//!
//! Represents a field that can be unchanged, explicitly cleared, or set to a new value.
...
pub enum Clearable<T> {
    ...
    #[default]
    Unchanged,
    /// Explicitly clear the value to None.
    Clear,
    /// Set to a new value.
    Set(T),
}
...
impl<T> Clearable<T> {
    /// Resolve this clearable with an existing value.
    ...
    /// Check if the value is cleared.
    pub fn is_clear(&self) -> bool {
        matches!(self, Clearable::Clear)
    }

    /// Check if the value is set.
    pub fn is_set(&self) -> bool {
        matches!(self, Clearable::Set(_))
    }
}
